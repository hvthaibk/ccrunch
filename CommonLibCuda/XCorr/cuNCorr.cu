...
cuNCorrSingle::cuNCorrSingle(size_t refNrowInput,
                             size_t refNcolInput,
                             size_t refNsecInput)
{
...
cuNCorrSingle::~cuNCorrSingle()
{
}

void cuNCorrSingle::memAlloc(void)
{
    // memory allocation
...
void cuNCorrSingle::memFree(void)
{
    // memory deallocation
...
void cuNCorrSingle::prepareRef(float *refDataInput)
{
...
void cuNCorrSingle::setSizeTpl(size_t tplNrowInput,
                               size_t tplNcolInput,
                               size_t tplNsecInput)
{
...
void cuNCorrSingle::copyTplMsk(float *tplDataInput, float *mskDataInput)
{
    // template
...
void cuNCorrSingle::normalizeTpl(size_t mskAreaInput)
{
    if (mskAreaInput > 0) {
        mskArea = mskAreaInput;
    }
    else {
        mskArea = cuda_xcorrNormTemplateNCC(dev_tplData, tplSize, dev_mskData);
    }
}

void cuNCorrSingle::prepareTplMsk(void)
{
    if (refNsec == 1) {       // 2D
        // template
...
void cuNCorrSingle::computeCorr(void)
{
    // xcorr 1
...
float cuNCorrSingle::getResultMax(void)
{
    if (refNsec == 1) {       // 2D
...
float* cuNCorrSingle::getResultRef(void)
{
    return dev_resDataPadded;
}

void cuNCorrSingle::getResult(float *resDataPaddedOut)
{
    cuda_array_memcpy_d2h(resDataPaddedOut, dev_resDataPadded, refSize);
}

void cuNCorrSingle::mergeResult(size_t indx, eXCorrMerge bAbs)
{
...
void cuNCorrSingle::mergeResultGlobal(float  *resDataAbsMaxPaddedGlobal,
...
void cuNCorrSingle::copyTplMskRot3D(float *tplDataInput, float *mskDataInput)
{
    assert(tplSize > 0);
...
void cuNCorrSingle::tplCreateByRot3D(float alpha, float beta, float gamma, eInter inter)
{
    assert(tplSize > 0);
...
void cuNCorrSingle::mskCreateByRot3D(float alpha, float beta, float gamma, eInter inter)
{
    assert(tplSize > 0);
...
cuNCorrDouble::cuNCorrDouble(size_t refNrowInput,
                             size_t refNcolInput,
                             size_t refNsecInput)
{
    assert(refNrowInput > 0 && refNcolInput > 0 && refNsecInput > 0);
...
cuNCorrDouble::~cuNCorrDouble()
{
}

void cuNCorrDouble::memAlloc(void)
{
    // memory allocation
    cuda_arrayDev_new(dev_tplDataPadded, refSize);
    cuda_arrayDev_new(dev_refDataPadded, refSize);
...
void cuNCorrDouble::memFree(void)
{
    // memory deallocation
    if (dev_tplData != NULL)  cuda_arrayDev_delete(dev_tplData);
    if (dev_mskData != NULL)  cuda_arrayDev_delete(dev_mskData);
...
void cuNCorrDouble::prepareRef(double *refDataInput)
{
    cuda_array_memcpy_h2d(dev_refDataPadded, refDataInput, refSize);
...
void cuNCorrDouble::setSizeTpl(size_t tplNrowInput,
                               size_t tplNcolInput,
                               size_t tplNsecInput)
{
    assert(tplNrowInput > 0 && tplNcolInput > 0 && tplNsecInput > 0);
...
void cuNCorrDouble::copyTplMsk(double *tplDataInput, double *mskDataInput)
{
    // template
#ifdef __GEM_USE_CUDA_PINNEDMEM__
    CUDA_SAFE_CALL( cudaMemcpyAsync(dev_tplData,
...
void cuNCorrDouble::normalizeTpl(size_t mskAreaInput)
{
    if (mskAreaInput > 0) {
        mskArea = mskAreaInput;
    }
...
void cuNCorrDouble::prepareTplMsk(void)
{
    if (refNsec == 1) {       // 2D
        // template
        cuda_array_pad(dev_tplData,       tplNrow, tplNcol,
...
void cuNCorrDouble::computeCorr(void)
{
    // xcorr 1
    cuda_xcorrModulateAndNormalize
        (dev_tplDataPaddedFFT,
...
double cuNCorrDouble::getResultMax(void)
{
    if (refNsec == 1) {       // 2D
        return cuda_array_reduce_min(dev_resDataPadded, refSize);
    }
...
double* cuNCorrDouble::getResultRef(void)
{
    return dev_resDataPadded;
}

void cuNCorrDouble::getResult(double *resDataPaddedOut)
{
    cuda_array_memcpy_d2h(resDataPaddedOut, dev_resDataPadded, refSize);
}

void cuNCorrDouble::mergeResult(size_t indx, eXCorrMerge bAbs)
{
    cuda_xcorrMergeResult(dev_resDataPadded,
                          dev_resDataAbsMaxPadded,
                          dev_resDataMaxIndPadded,
...
void cuNCorrDouble::mergeResultGlobal(double *resDataAbsMaxPaddedGlobal,
                                      size_t *resDataMaxIndPaddedGlobal,
                                      eXCorrMerge bAbs)
{
    cuda_array_memcpy_d2h(resDataAbsMaxPadded, dev_resDataAbsMaxPadded, refSize);
...
void cuNCorrDouble::copyTplMskRot3D(double *tplDataInput, double *mskDataInput)
{
    assert(tplSize > 0);
    assert(tplDataInput != NULL);
    assert(mskDataInput != NULL);
...
void cuNCorrDouble::tplCreateByRot3D(double alpha, double beta, double gamma, eInter inter)
{
    assert(tplSize > 0);
    assert(dev_tplDataRotInit != NULL && dev_tplData != NULL);
...
void cuNCorrDouble::mskCreateByRot3D(double alpha, double beta, double gamma, eInter inter)
{
    assert(tplSize > 0);
    assert(dev_mskDataRotInit != NULL && dev_mskData != NULL);
